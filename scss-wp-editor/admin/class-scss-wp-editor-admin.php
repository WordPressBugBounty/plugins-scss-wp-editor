...
class Scss_Wp_Editor_Admin {
	...
	public function enqueue_styles( $hook_suffix ) {
		 ...
		 * class.
		 */
		if ( 'settings_page_scss-wp-editor' == $hook_suffix || $hook_suffix === 'settings_page_scss-wp-editor-network' ) {
	...
	/**
	 * Created Network Admin Menu.
	 *
	 * @since    1.2.0
	 */
	public function swe_network_admin_menu() {
//            var_dump(is_multisite());var_dump(is_plugin_active_for_network( plugin_basename( __FILE__ ) ));print_r(plugin_basename( __FILE__ ));die("!");
            if ( is_multisite() && is_plugin_active_for_network( SCSS_WP_EDITOR_PLUGIN_BASENAME ) ) {
		add_submenu_page(
			'settings.php',
			__( 'Network SCSS Editor', 'scss-wp-editor' ),
			__( 'Network SCSS Editor', 'scss-wp-editor' ),
			'manage_network_options',
			'scss-wp-editor-network',
			array( &$this, 'swe_editor_network_page' )
		);
            }
	}
	 ...
	 * @since    1.2
	 */
	public function swe_editor_network_page() {

	    if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {

			$scssVal = sanitize_textarea_field( wp_unslash( $_POST['scss-val'] ) );
			update_site_option( 'swe_scss_box_value', $scssVal );

			$compiledCss = $this->swe_scss_compiler( $scssVal );
			$compiledCss = html_entity_decode( $compiledCss );

			$uploadsDir = trailingslashit( wp_upload_dir()['basedir'] ) . 'scss-wp-network';

			if ( ! file_exists( $uploadsDir ) ) {
				wp_mkdir_p( $uploadsDir );
			}

			$uploadFile = $uploadsDir . '/compiled-scss.css';

			if ( is_object( $compiledCss ) ) {

				update_site_option( 'swe_box_value_status', 'fail' );
				...
				// Success
				update_site_option( 'swe_box_value_status', 'success' );
				file_put_contents( $uploadFile, html_entity_decode( $compiledCss, ENT_QUOTES, 'cp1251') );
		...
		$swe_scss_box_value = get_site_option( 'swe_scss_box_value' );
		$val = $swe_scss_box_value ? $swe_scss_box_value : '';
		$this->swe_editor_page_html($val);
	}
	 ...
	 * @since    1.0.0
	 */
	public function swe_editor_page_html($editor_content) {
		?>
		<div class="wrap">
				    	...
				    	<div class="swp-tabs">
							...
							<div class="tab-content current" id="tab-01">
								<div class="swp-editor-block">
									...
									<textarea id="fancy-textarea" name="scss-val"><?php echo esc_textarea( $editor_content ); ?></textarea>
								</div>
							</div>
								...
								<table class="form-table">
									<tbody>
										<tr>
											...
											<td>
												<p class="description">
													If you like the plugin and helps you, give us a <a href="https://wordpress.org/support/plugin/scss-wp-editor/reviews/#new-post" target="_blank">review
													<?php
														for ( $i = 1; $i <= 5 ; $i++ ) { 
